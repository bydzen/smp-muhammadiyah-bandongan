<?php
    ...
    if(isset($_GET['idekstra'])){

        $ekstra = mysqli_query($conn, "SELECT gambar FROM ekstra WHERE id = '".$_GET['idekstra']."'");

        if(mysqli_num_rows($ekstra)>0){
            $p = mysqli_fetch_object ($ekstra);

            if(file_exists("../uploads/ekstra/".$p->gambar)){
                unlink ("../uploads/ekstra/".$p->gambar);
            }
        }
        $delete = mysqli_query($conn, "DELETE FROM ekstra WHERE id = '".$_GET['idekstra']."'");
